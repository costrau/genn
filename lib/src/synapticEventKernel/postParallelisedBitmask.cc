...
//----------------------------------------------------------------------------
int SynapticEventKernel::PostParallelisedBitmask::getCompatibility(const SynapseGroup &sg) const
{
    ...
    // Not compatible with non-bitmask matrices
    if(!(sg.getMatrixType() & SynapseMatrixConnectivity::BITMASK)) {
        return -1;
    }

    // Not compatible with non-global weights
...
//----------------------------------------------------------------------------
void SynapticEventKernel::PostParallelisedBitmask::generateInnerLoop(
    ...
    const string &ftype) const
{
    ...
    const int uintBits = sizeof(unsigned int) * 8;
    const int logUintBits = (int)(logf((float)uintBits) / logf(2.0f) + 1e-5f);
    ...
    os << "const unsigned int gid = (shSpk" << postfix << "[j] * " << sg.getTrgNeuronGroup()->getNumNeurons() << " + lid);" << std::endl;

    if (!wu->getSimSupportCode().empty()) {
        os << "using namespace " << sg.getName() << "_weightupdate_simCode;" << std::endl;
    }
    if (evnt && sg.isEventThresholdReTestRequired()) {
        os << "if ((B(dd_gp" << sg.getName() << "[gid >> " << logUintBits << "], gid & " << uintBits - 1 << ")) && ";
        // code substitutions ----
    ...
    os << "if (B(dd_gp" << sg.getName() << "[gid >> " << logUintBits << "], gid & " << uintBits - 1 << "))" << CodeStream::OB(135);
